...
getOrgChart.prototype._8 = function() {
    this._zm = get._r().msie ? this.element.clientWidth : window.getComputedStyle(this.element, null).width;
    this._zm = parseInt(this._zm);
    if (this._zm < 3) {
        this._zm = 1024;
        this.element.style.width = "1024px"
    }
    this._zj = get._r().msie ? this.element.clientHeight : window.getComputedStyle(this.element, null).height;
    this._zj = parseInt(this._zj);
    if (this._zj < 3) {
        this._zj = parseInt((this._zm * 9) / 16);
        this.element.style.height = this._zj + "px"
    }
    this._aT = this._zm;
    this._aV = this._zj - this.theme.toolbarHeight;
...
getOrgChart.prototype.changeColorScheme = function(a) {
    ...
    this._p._zu.className = this._p._zu.className.replace(this.config.color, a);
    this.config.color = a
};
...
getOrgChart.prototype._L = function() {
    if (this.config.scale === "auto") {
        ...
        var i = e - (this.config.siblingSeparation / 2);
        var j = f - (this.config.levelSeparation / 2);
        switch (this.config.orientation) {
            case getOrgChart.RO_TOP:
            case getOrgChart.RO_TOP_PARENT_LEFT:
                j = (this.config.levelSeparation / 2);
                break;
            case getOrgChart.RO_BOTTOM:
            case getOrgChart.RO_BOTTOM_PARENT_LEFT:
                break;
            ...
            case getOrgChart.RO_LEFT:
            case getOrgChart.RO_LEFT_PARENT_TOP:
                i = (this.config.siblingSeparation / 2);
                break
        }
        var h = Math.abs(e) + Math.abs(c) + this.theme.size[0] + this.config.siblingSeparation;
        var a = Math.abs(f) + Math.abs(d) + this.theme.size[1] + this.config.levelSeparation;
        this.initialViewBoxMatrix = [i, j, h, a]
    } else {
        var i = this.config.siblingSeparation / 2;
        var j = this.config.levelSeparation / 2;
        var h = (this._aT) / this.config.scale;
        var a = (this._aV) / this.config.scale;
        switch (this.config.orientation) {
            case getOrgChart.RO_TOP:
            case getOrgChart.RO_TOP_PARENT_LEFT:
                this.initialViewBoxMatrix = [-i, j, h, a];
            ...
            case getOrgChart.RO_BOTTOM:
            case getOrgChart.RO_BOTTOM_PARENT_LEFT:
                this.initialViewBoxMatrix = [-i, -j - a, h, a];
                break;
            case getOrgChart.RO_RIGHT:
            case getOrgChart.RO_RIGHT_PARENT_TOP:
                this.initialViewBoxMatrix = [-h - j, -i, h, a];
                break;
            case getOrgChart.RO_LEFT:
            case getOrgChart.RO_LEFT_PARENT_TOP:
                this.initialViewBoxMatrix = [j, -i, h, a];
                break
        }
    }
...
getOrgChart.prototype.draw = function(a) {
    ...
    if (this.config.enableZoom) {
        this._p._zA.style.display = "inherit";
        this._p._zp.style.display = "inherit"
    }
...
getOrgChart.prototype._w = function(a) {
    ...
    for (var d in this.nodes) {
        if (this.nodes[d]._zk == null || this.nodes[d]._zo == null) {
            continue
        }
        if (this.nodes[d]._zk == this.nodes[d].x && this.nodes[d]._zo == this.nodes[d].y) {
            continue
        }
...
getOrgChart.prototype._d = function() {
    ...
    if (this.config.enableZoom) {
        this._a(this._p._zp, "click", this._zQ);
        this._a(this._p._zA, "click", this._zZ);
...
getOrgChart._p.prototype._aN = function() {
    this._zu = this.element.getElementsByTagName("div")[0];
    var a = this._zu.children;
    this._zt = a[0];
    this._v = a[1];
    this._b = a[2];
    this._P = a[3]
...
getOrgChart._p.prototype._aH = function() {
    ...
    this._a1 = this._zu.getElementsByClassName("get-right")[0];
    this._av = this._zu.getElementsByClassName("get-left")[0];
    this._i = this._zu.getElementsByClassName("get-down")[0];
    this._zh = this._zu.getElementsByClassName("get-up")[0]
};
...
getOrgChart.node.prototype.draw = function(a) {
    ...
    h.push(getOrgChart.OPEN_NODE.replace("[data-node-id]", this.id).replace("[x]", this._zk == null ? this.x : this._zk).replace("[y]", this._zo == null ? this.y : this._zo).replace("[level]", this.level).replace("[nodeCssClass]", d));
...
get._w = function(a, c, b, h, j, d) {
    ...
    function f() {
        for (var s in b) {
            var t = getOrgChart.util._zi(["top", "left", "right", "bottom"], s.toLowerCase()) ? "px" : "";
...
getOrgChart.util._zi = function(a, c) {
    var b = a.length;
    while (b--) {
        if (a[b] === c) {
            return true
...
getOrgChart.prototype._zQ = function() {
    this.zoom(1, true)
};
getOrgChart.prototype._zZ = function() {
    this.zoom(-1, true)
};
...
getOrgChart.prototype._t = function(c, e, b, a) {
    ...
    if (this.nodes[c]) {
        d._zk = this.nodes[c].x;
        d._zo = this.nodes[c].y
    } else {
        d._zk = null;
        d._zo = null
    }
    this.nodes[c] = d;
    for (label in d.data) {
        if (!getOrgChart.util._zi(this._af, label)) {
            this._af.push(label)
        }
    }
    return d
